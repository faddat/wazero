...
func TestInstruction_String(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			i: &instruction{
				kind: condBr,
				u1:   registerAsRegNonZeroCond(regToVReg(x29)).asUint64(),
		...
		},
		{
			i: &instruction{
				...
				rd:   operandNR(backend.VReg(0)),
			},
			exp: "ldr r0?, pc+8; b 8; data.f32 3.000000",
		},
		{
			i: &instruction{
				kind: loadFpuConst64,
			...
			exp: "ldr r0?, pc+8; b 16; data.f64 12345.987491",
		},
