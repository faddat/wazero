...
func (m *machine) lowerConstantI64(dst backend.VReg, c int64) {
	// Following the logic here:
...
func (m *machine) load64bitConst(c int64, dst backend.VReg) {
	var bits [4]uint64
	var zeros, negs int
	for i := 0; i < 4; i++ {
...
func (m *machine) insertMOVZ(dst backend.VReg, v uint64, shift int, dst64 bool) {
	instr := m.allocateInstr()
	instr.asMOVZ(dst, v, uint64(shift), dst64)
	m.insert(instr)
}

func (m *machine) insertMOVK(dst backend.VReg, v uint64, shift int, dst64 bool) {
	instr := m.allocateInstr()
	instr.asMOVK(dst, v, uint64(shift), dst64)
	m.insert(instr)
}

func (m *machine) insertMOVN(dst backend.VReg, v uint64, shift int, dst64 bool) {
	instr := m.allocateInstr()
